...
const loader = faceapi.nets.tinyFaceDetector.load('./')
	.then(async ()=>{
		//Dry run
...
self.onmessage = async(event)=>{
	...
	switch(event.data.cmd)
	{
		...
		case "frame":
		{
			...
			try {
				//get image size
				const displaySize = { width: event.data.width,  height: event.data.height };
				//Change canvas size
				faceapi.matchDimensions(canvas, displaySize)
				//Paint in canvas
				canvas.width = event.data.width;
				canvas.height = event.data.height;
				canvas.getContext('bitmaprenderer').transferFromImageBitmap(event.data.frame);
				//Deted face
				const result = await faceapi.detectSingleFace (canvas, options)
				...
				if (result)
				{
					...
					faceapi.draw.drawDetections(overlay, faceapi.resizeResults(result, displaySize));
				}
